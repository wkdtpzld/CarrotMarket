...
const ChatDetail: NextPage = () => {
  ...
  const [reservation, { data: reservationData, loading: reservationLoading }] =
    useMutation<ReservationResponse>(
      `/api/chat/${router.query.id}/reservation`,
      "POST"
    );
  ...
  // Message 관련
  const [createMessage, { data: MessageData, loading }] = useMutation(
    `/api/chat/${router.query.id}/message`,
    "POST"
  );
            ...
            <div className="space-x-4 ml-4 flex ">
              ...
              <div className="text-sm flex flex-col">
                <span className="text-gray-700">
                  {data.messages.product?.name}
                </span>
                <span className="text-xl font-semibold">
                  $ {data.messages.product?.price}
                </span>
              </div>
            </div>
          ...
          <div className="px-4 py-10 space-y-4 my-14">
            {data?.messages.message?.map((message) => (
              <ChattingBubble
                ...
                type={message.user.id === data.user.id ? "inside" : "opposite"}
                imageId={
                  message?.user?.loginType === "default"
            ...
            {data.messages.product?.reservation ? (
              <>
                {new Date(
                  data?.messages.product?.reservation.reservationDate!
                ) > new Date() ? (
                  <div className="flex items-center justify-center mt-6 relative">
                    ...
                    <span className="text-sm">일 거래 예약날입니다.</span>
                  </div>
                ) : (
                  <>
...
const Page: NextPage<{ok: boolean, messages: Message, user: User}> = ({ ok, messages, user }) => {
